...
public class HashMapII {
     ...
     * @return the value to which the specified key is mapped, or Integer.MIN_VALUE if this map contains no mapping for the key
     */
    ...
    public int[] keysArray() {
        ...
        int ix = 0;
        for (int i = 0; i < keys.length; i++) {
            if (keys[i] != Integer.MIN_VALUE) result[ix++] = keys[i];
        }
        return result;
    }
    ...
    private class KeyIterator implements IteratorI {
        ...
        private void skip() {
            while (i < keys.length && keys[i] == Integer.MIN_VALUE) i++;
        }
    }
}
