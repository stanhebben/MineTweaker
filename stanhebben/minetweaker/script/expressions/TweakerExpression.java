...
public abstract class TweakerExpression {
	...
	private static TweakerExpression readPrimaryExpression(TweakerFile file, int line, int offset, TweakerParser parser) {
		switch (parser.peek().getType()) {
			...
			case T_DOLLAR: {
				TweakerExpression result = new TweakerExpressionDollar(file, line, offset);
			...
			case T_FUNCTION:
				...
				if (parser.optional(T_BRCLOSE) != null) {
					...
					argumentNamesAL.add(parser.required(T_ID, "identifier expected").getValue());
